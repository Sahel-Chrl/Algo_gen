...
parab descendant(parab mere, float variance) 
{
	...
	{
		enfant[i] = mere[i] + variance * hasard1Neg(); //enfant + mutation
	}
	return enfant;
}
...
int main()
{
	...
	for (int i = 0; i < generations; i++)
	{
		...
		for(int j=0; j<popu.size(); j++){ //on cree une nouvelle population du même nombre d'individu que la precedente
				...
				float variance=10; 
				if(scores[indiv]/100<10){variance=scores[indiv]/100;}//plus l'individu est bien moins on le fait muter mais la maniere de la faire peut etre a revoir
